...
export function useSocketRooms() {
  ...
  const connect = () => {
    ...
    newSocket.on(ServerSocketResposneEvent.JOIN_SUCCESS, (data) => {
      ...
      const currentClientInfo = updatedRoomInfo.members.filter(
        (member) => member.id === updatedRoomInfo.myId
      )[0];
      setClient(() => currentClientInfo);
    });
  ...
  const joinRoom = (roomId: string, token: string) => {
    if (roomId) {
      socket?.emit(ClientSocketEvent.JOIN_ROOM, {
        roomId: roomId,
        token: token,
      });
    }
  ...
  const getHostNickName = () => {
    const host = roomInfo?.members.filter(
      (member) => member.id === roomInfo.hostId
    )[0];
    return host?.nickname || "";
  };
